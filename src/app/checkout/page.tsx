...
export default function CheckoutPage() {
  ...
  const handleOrderSubmit = async () => {
    if (!file || !formData.transactionId) {
      alert(t('checkout.uploadAndTransactionRequired'));
      return;
    }
    ...
    try {
      ...
      if (!uploadJson.success) {
        throw new Error(t('checkout.failedToUploadProof'));
      }
      ...
      if (!orderJson.success) {
        throw new Error(orderJson.message || t('checkout.failedToCreateOrder'));
      }

      // Clear cart and redirect
      clearCart();
      const formattedId = `#ORD-${orderJson.data.orderId.toString().padStart(4, '0')}`;
      alert(t('checkout.orderPlaced').replace('{orderId}', formattedId));
      router.push('/');
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!paymentStep ? (
          ...
          <div className="bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-neutral-100">
            <h2 className="text-2xl font-bold text-foreground mb-8">{t('checkout.shippingAddress')}</h2>
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setPaymentStep(true); }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.phoneNumber')}</label>
                  <input
                    type="tel"
                    name="phone"
              ...
              <div>
                <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.emailOptional')}</label>
                <input
                  type="email"
                  name="email"
              ...
              <div>
                <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.completeAddress')}</label>
                <textarea
                  rows={3}
                  name="address"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.city')}</label>
                  <input
                    type="text"
                    name="city"
                    value={formData.city}
                ...
                <div>
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.state')}</label>
                  <input
                    type="text"
                    name="state"
                ...
                <div className="col-span-2 md:col-span-1">
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.pincode')}</label>
                  <input
                    type="text"
                    name="pincode"
              ...
              <div className="pt-8 flex justify-between items-center border-t border-neutral-100 mt-8">
                <Link href="/cart" className="text-primary font-bold hover:underline">{t('checkout.returnToCart')}</Link>
          ...
          <div className="bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-neutral-100">
            <div className="text-center mb-10">
              <h2 className="text-3xl font-bold text-primary mb-2">{t('checkout.scanAndPay')}</h2>
              <p className="text-neutral-600 font-inter">{t('checkout.totalAmountToPay')}: <strong className="text-secondary text-xl">₹{total.toFixed(2)}</strong></p>
            </div>
              ...
              <div className="flex-1 w-full space-y-6">
                <div className="bg-primary/5 p-6 rounded-2xl border border-primary/20 mb-6">
                  <h3 className="font-bold text-primary mb-2 flex items-center gap-2">
                    ...
                    {t('checkout.paymentInstructions.title') || 'Payment Instructions'}
                  </h3>
                  <ol className="list-decimal list-inside text-sm text-neutral-700 space-y-2 font-inter">
                    <li>{t('checkout.paymentInstructions.step1')}</li>
                    <li>{t('checkout.paymentInstructions.step2').replace('{amount}', `₹${total.toFixed(2)}`)}</li>
                    <li>{t('checkout.paymentInstructions.step3')}</li>
                    <li>{t('checkout.paymentInstructions.step4')}</li>
                  </ol>
                </div>

                <div>
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.upiTransactionId')}</label>
                  <input
                    type="text"
                    name="transactionId"
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                    placeholder={t('checkout.exampleTransactionPlaceholder')}
                    required
                  />
                </div>

                <div>
                  <label className="block text-sm font-bold text-foreground mb-2">{t('checkout.uploadPaymentScreenshot')}</label>
            ...
            <div className="pt-8 flex justify-between items-center border-t border-neutral-100">
              <button type="button" onClick={() => setPaymentStep(false)} className="text-neutral-500 font-bold hover:text-primary transition-colors">{t('common.back')}</button>
              <button
                type="button"
              ...
              >
                {loading ? t('checkout.processingOrder') : t('checkout.confirmOrder')}
              </button>
            </div>
          </div>
        )}
