    ...
    <main className="flex-1 bg-surface py-12 md:py-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4 mb-10 border-b border-neutral-200 pb-6">
           <h1 className="text-3xl lg:text-4xl font-bold text-foreground tracking-tight">{t('cart.shoppingCart')}</h1>
        ...
        {cartItems.length === 0 ? (
          <div className="bg-white p-16 rounded-3xl shadow-sm border border-neutral-100 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-4">{t('cart.emptyCart')}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                {cartItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="bg-white p-5 rounded-3xl shadow-sm border border-neutral-100 flex flex-col sm:flex-row items-center gap-6 group hover:shadow-md transition-shadow">
                       <Link href={`/products/${item.slug}`} className="w-28 h-28 bg-neutral-50 rounded-2xl flex items-center justify-center shrink-0 relative overflow-hidden border-4 border-white shadow-sm">
                          <Image src={item.image} alt={displayName} fill className="object-cover p-2" />
                       </Link>
                       <div className="flex-1 text-center sm:text-left w-full">
                          <Link href={`/products/${item.slug}`}>
                            <h3 className="text-lg font-bold text-foreground mb-1 hover:text-primary transition-colors">{displayName}</h3>
                          </Link>
                          <p className="text-neutral-500 font-inter text-sm mb-3 bg-neutral-50 inline-block px-3 py-1 rounded-md">{t('cart.size')}: {item.size}</p>
                       ...
                       <div className="flex items-center gap-4 border-t sm:border-t-0 border-neutral-100 w-full sm:w-auto pt-4 sm:pt-0 justify-center">
                          ...
                          <button onClick={() => removeItem(item.id)} className="p-3 text-neutral-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors shrink-0" title={t('cart.removeItem')}>
             ...
             <div className="bg-white p-8 rounded-3xl shadow-sm border border-neutral-100 h-fit sticky top-28">
                <h2 className="text-xl font-bold text-foreground mb-6 pb-4 border-b border-neutral-100">{t('cart.orderSummary')}</h2>
